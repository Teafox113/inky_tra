...
| 預定發布檔名 | `Inky-Translation-Fork-v1.8.0-dev.4-windows-x64.zip` |
...
- Ink 行類型解析、部分標籤保護；CSV 詞彙表編輯、分類及 AI 擷取候選詞。
- 搜尋取代及 AI 助理面板；助理可使用角色預設、附加文本、記憶與人格設定。
- 預設「霧港十三夜」教學：角色建立、點數分配、三層分支、物品與四種結局；另保留「雨天書店」及英文翻譯練習。

上述既有功能已匯入；完整互動驗證仍在進行如有任何BUG請聯繫。

問題回報：[GitHub Issues](https://github.com/Teafox113/inky_tra/issues)。

## 開啟 Windows 測試版

解壓 Inky-Translation-Fork-v1.8.0-dev.4-windows-x64.zip 後，雙擊 Inky-Translation-Fork.exe。首次啟動預設繁體中文並開啟「霧港十三夜」。舊版使用者升級後也展示一次；可從 **範例 → 開啟範例：霧港十三夜（角色與分支教學）** 重新開啟。ZIP 中也有可直接瀏覽的 **範例** 資料夾。
...
本倉庫提供 1.8.0-dev.4 開發版原始碼。Windows 測試包可依下方建置命令自行產生；目前尚未建立可下載的 GitHub Release。
...
開啟 **翻譯 → 詞彙表管理／AI 建表**，或在 **翻譯設定 → 詞彙表路徑** 旁按管理按鈕。

1. 開啟既有 CSV 或新建表格；填入原文、譯文、分類與備註。分類支援角色、地點、組織、物品、技能、系統用語及自訂名稱。
2. 需要 AI 協助時，先儲存翻譯 API／模型設定，再選「目前編輯檔案」「選取文字」或「目前專案已載入檔案」，按 **AI 擷取專有詞彙**。
3. 確認傳送範圍、服務與模型後才發送。每批最多 8,000 字元，每次最多 240,000 字元；可停止後續批次，已取得結果會保留。
4. 檢查 AI 建議的譯名與分類，勾選 **採用**，再按 **儲存並套用**。之後目前檔案、選取文字及專案翻譯會使用這份詞彙表。

CSV 欄位為 `en_term,zh_term,category,notes,status`；`status` 是 `pending`（待確認）或 `approved`（採用）。舊兩欄／四欄表格仍可讀取，預設視為已採用。檔案為 UTF-8，支援引號、逗號與欄內換行。

每個專案可另存一份 glossary.csv，但目前使用的路徑是共用翻譯設定；切換專案時請確認詞彙表。儲存覆蓋前保留 .bak_時間戳 備份。AI 遇到重複原文會保留原有譯名，待確認項目不參與翻譯。開啟或編輯表格本身不呼叫 AI。

AI 掃描會傳送所選範圍的完整文字，包含未儲存內容與程式碼／註解；不會自動讀取整個磁碟。可能產生 API 費用，已回報用量會計入統計；未提供價格時金額顯示為 0 不代表免費。遠端掃描 API 要求 HTTPS，本機服務可用 HTTP。
